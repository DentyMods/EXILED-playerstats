...
namespace PlayerStatsBot
{
  public class Bot
  {
    ...
    private async Task HandleCommand(ICommandContext context)
    {
      ...
      {
        IUserMessage userMessage = await context.Channel.SendMessageAsync("Pong! 🏓 **" + _client.Latency + "ms**");
      }
    ...
    private async Task CheckTeslaStats(ICommandContext context)
    {
      ...
      embed.WithFooter("Stats bot - Joker11");
      embed.WithCurrentTimestamp();
      embed.WithTitle("Axuka Servers Tesla Gate Stats");
          ...
          if (user != null)
          {
            ...
            else
            {
              ...
              else
              {
                ...
                embed.WithTitle("Player Stats for: " + steamName + "(" + steamId + ")");
                embed.AddField("Total Kills", stats.Kills);
                embed.AddField("Total SCP Kills", stats.ScpKills);
                embed.AddField("Deaths", stats.Deaths);
                embed.AddField("Suicides", stats.Suicides);
                embed.AddField("SCP-207 Drinks", stats.Scp207Uses);
                embed.AddField("SCP-018 Throws", stats.Scp018Throws);
                embed.AddField("Last Killed", stats.LastKiller);
                embed.AddField("Last Victim", stats.LastVictim);
                embed.AddField("KDR", stats.Krd);
                embed.AddField("Escapes Made", stats.Escapes);
                embed.AddField("Rounds Played", stats.RoundsPlayed);
                embed.AddField("Time Played", GetHRT(stats.SecondsPlayed));
                embed.WithFooter(f => f.Text = "Stats Bot Made by Joker119").WithColor(Color.Blue)
